...
func CreateSparseFloatRow(indices []uint32, values []float32) []byte {
	...
	row := make([]byte, elemCount*8)
	for i := 0; i < elemCount; i++ {
		SparseFloatRowSetAt(row, i, sortedIndices[i], sortedValues[i])
	}
	return row
}
...
func CreateSparseFloatRowFromJSON(input map[string]interface{}) ([]byte, error) {
	...
	if !ok1 && !ok2 {
		for k, v := range input {
			...
			if !ok {
				...
				if !ok {
					return nil, fmt.Errorf("invalid float32 in JSON")
				}
	...
	row := CreateSparseFloatRow(indices, values)
