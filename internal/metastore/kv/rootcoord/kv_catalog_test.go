...
func TestCatalog_TempCollectionExchange(t *testing.T) {
	kvmock := mocks.NewSnapShotKV(t)
	kvmock.EXPECT().MultiSaveAndRemoveWithPrefix(mock.Anything, mock.Anything, mock.Anything).Return(nil).Once()
	c := &Catalog{Snapshot: kvmock}
	ctx := context.Background()
	err := c.ExchangeCollections(ctx, []*model.Collection{}, []*model.Alias{}, uint64(1))
	assert.NoError(t, err)
	err = c.ExchangeCollections(ctx, []*model.Collection{
		{
			CollectionID: 1,
			Name:         "tempColl",
			State:        pb.CollectionState_CollectionCreated,
