...
func newIndexServer(catalog *catalogmocks.DataCoordCatalog, b *mocks.MockRootCoordClient) *Server {
	return &Server{
		ctx: context.Background(),
		meta: &meta{
			catalog: catalog,
...
func TestServer_CreateIndexesForTemp(t *testing.T) {
	...
	t.Run("create indexes for temp collection success", func(t *testing.T) {
		s.stateCode.Store(commonpb.StateCode_Healthy)
		catalog := catalogmocks.NewDataCoordCatalog(t)
		catalog.EXPECT().CreateIndex(mock.Anything, mock.Anything).Return(nil).Times(3)
		s.meta = &meta{catalog: catalog, indexMeta: &indexMeta{
			catalog: catalog, indexes: map[UniqueID]map[UniqueID]*model.Index{collID: {
				// finished
				indexID: {
...
func TestServer_TempCollectionIndexes(t *testing.T) {
	paramtable.Init()
	catalog := catalogmocks.NewDataCoordCatalog(t)
	catalog.EXPECT().CreateIndex(mock.Anything, mock.Anything).Return(nil).Times(7)
	catalog.EXPECT().DropIndex(mock.Anything, mock.Anything, mock.Anything).Return(nil).Twice()
	...
	s := newIndexServer(catalog, b)
	s.broker = broker.NewCoordinatorBroker(b)
	s.stateCode.Store(commonpb.StateCode_Healthy)
	s.allocator.allocN(100)

	s.CreateIndexesForTemp(s.ctx, &indexpb.CollectionWithTempRequest{
		CollectionID:     collID,
		TempCollectionID: collID - 1,
	})
	...
	s.DropIndexesForTemp(s.ctx, &indexpb.CollectionWithTempRequest{
		CollectionID:     collID,
		TempCollectionID: collID - 1,
	})
	assert.NoError(t, err)
