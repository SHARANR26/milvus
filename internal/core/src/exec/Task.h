...
class Task : public std::enable_shared_from_this<Task> {
 public:
    ...
    void
    Start(uint32_t max_drivers, uint32_t concurrent_splitgroups);
    ...
    void
    CreateAndStartDrivers(uint32_t concurrent_split_group);

    void
    CreateDriverFactoryLocked(uint32_t max_drivers);
 ...
 private:
    ...
    // std::vector<std::shared_ptr<ExchangeClient>> exchange_clients_;

    // std::unordered_map<milvus::plan::PlanNodeId,
    //                    std::shared_ptr<ExchangeClient>>
    //     exchange_client_by_plannode_;
