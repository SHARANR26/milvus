...
func (t *mixCompactionTask) statSerializeWrite(ctx context.Context, buffer *writebuffer.SegmentInsertBuffer, finalRowCount int64) (*datapb.FieldBinlog, error) {
	ctx, span := otel.Tracer(typeutil.DataNodeRole).Start(ctx, "statslog serializeWrite")
	defer span.End()
	sblob, err := buffer.Finish(finalRowCount)
	if err != nil {
		return nil, err
	}
	...
	key, _ := binlog.BuildLogPath(storage.StatsBinlog, buffer.GetCollectionID(), buffer.GetPartitionID(), buffer.GetSegmentID(), buffer.GetPkID(), logID)
	kvs := map[string][]byte{key: sblob.GetValue()}
	statFieldLog := &datapb.FieldBinlog{
		FieldID: buffer.GetPkID(),
		Binlogs: []*datapb.Binlog{
			{LogSize: int64(len(sblob.GetValue())), LogPath: key, EntriesNum: finalRowCount},
		},
	}
...
func (t *mixCompactionTask) serializeWrite(ctx context.Context, buffer *writebuffer.SegmentInsertBuffer) (kvs map[string][]byte, fieldBinlogs map[int64]*datapb.FieldBinlog, err error) {
	...
	for i := range blobs {
		...
		key, _ := binlog.BuildLogPath(storage.InsertBinlog, buffer.GetCollectionID(), buffer.GetPartitionID(), buffer.GetSegmentID(), fID, startID+int64(i))
...
func (t *mixCompactionTask) merge(
	...
	buffer *writebuffer.SegmentInsertBuffer,
	...
	log := log.With(zap.Int64("planID", t.GetPlanID()), zap.Int64("compactTo segment", buffer.GetSegmentID()))
...
func (t *mixCompactionTask) Compact() (*datapb.CompactionPlanResult, error) {
	...
	buffer, err := writebuffer.NewSegmentInsertBuffer(t.metaCache.Schema(), previousRowCount, targetSegID, partID, t.metaCache.Collection())
	if err != nil {
		log.Warn("compact wrong, unable to init segment insert buffer", zap.Error(err))
		return nil, err
	}
	...
	compactToSeg, err := t.merge(ctxTimeout, allPath, deltaPk2Ts, buffer)
	if err != nil {
