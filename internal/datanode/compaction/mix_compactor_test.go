...
type MixCompactionTaskSuite struct {
	...
	segBuf *writebuffer.SegmentInsertBuffer
...
func (s *MixCompactionTaskSuite) TestCompactDupPK() {
	...
	for _, segID := range segments {
		...
		// InserBuffer will be cleared after serialize write, so get the pkfield before serializeWrite
		bfs.UpdatePKRange(s.segBuf.InsertBuffer.Yield().Data[Int64Field])

		kvs, fBinlogs, err := s.task.serializeWrite(context.TODO(), s.segBuf)
		s.Require().NoError(err)
...
func (s *MixCompactionTaskSuite) TestMergeNoExpiration() {
	...
	for _, test := range tests {
		s.Run(test.description, func() {
			...
			compactionSegment, err := s.task.merge(s.task.ctx, [][]string{lo.Keys(kvs)}, test.deletions, segInsertBuffer)
			s.NoError(err)
			s.NotNil(compactionSegment)
...
func (s *MixCompactionTaskSuite) initSegBuffer(magic int64) {
	row := getRow(magic)
	segBuf, err := writebuffer.NewSegmentInsertBuffer(s.meta.GetSchema(), 100, magic, PartitionID, CollectionID)
	s.Require().NoError(err)

	segBuf.BufferRow(row, storage.NewInt64PrimaryKey(magic), tsoutil.ComposeTSByTime(getMilvusBirthday(), 0))
	s.segBuf = segBuf
}
