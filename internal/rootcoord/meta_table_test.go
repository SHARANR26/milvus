...
func TestMetaTable_TempCollection(t *testing.T) {
	...
	t.Run("get temp collection fail", func(t *testing.T) {
		err := meta.ReplaceCollectionWithTemp(context.TODO(), "", testAliasName, typeutil.MaxTimestamp-1)
		assert.Error(t, err)
	})
	...
	t.Run("truncate fail", func(t *testing.T) {
		err := meta.ReplaceCollectionWithTemp(context.TODO(), "", testCollectionName, typeutil.MaxTimestamp-1)
		assert.Error(t, err)
	})
	...
	t.Run("truncate alias", func(t *testing.T) {
		meta.aliases.insert("", testAliasName, testCollectionID)
		err := meta.ReplaceCollectionWithTemp(context.TODO(), "", testAliasName, typeutil.MaxTimestamp-1)
		assert.Error(t, err)
	})
	t.Run("alter alias while truncating", func(t *testing.T) {
		meta.aliases.insert("", testAliasName, testCollectionID)
		meta.names.insert("", testCollectionName, testCollectionID)
		meta.names.insert("", util.GenerateTempCollectionName(testCollectionName), testCollectionID-1)
		err := meta.AlterAlias(context.TODO(), "", testAliasName, "", typeutil.MaxTimestamp-1)
		assert.Error(t, err)
	})
}
